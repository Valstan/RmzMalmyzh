...
Первый файл, который Codex читает в новой сессии проекта. Подсказывает, **где взять контекст** и **как правильно работать**.
...
Владелец убрал ритуал «окей на дифф/мерж/деплой»: подтверждение **заменено автоматическими гейтами**. Механизм — коммитимый `.agents/settings.json` (`defaultMode: auto` + allow/deny). Ярусно по риску:
...
| **Деплой на прод** (авто-триггер от merge; для docs/`.agents` не срабатывает — `paths-ignore`) | **авто** | смоук #011 в воркфлоу (200 + маркер + XFP-301); после деплоя дождаться зелёного |
