...
- ⚠️ Репозиторий приватный на тарифе без branch protection/rulesets: GitHub API
  отвечает 403, а `gh pr merge --auto` может смержить PR при pending checks.
  Поэтому порядок обязателен вручную: сначала `gh pr checks <PR> --watch`, убедиться,
  что `build` и `media-import` зелёные, и только затем `gh pr merge --squash`.
