...
- **Репозиторий публичный, `main` защищён сервером** (решение владельца D-030, исполнено brain 2026-08-17):
  прямой и force-push в `main` отклоняет сервер, правило действует и на администраторов
  (`enforce_admins`), а **обязательные проверки `build` и `media-import` блокируют влитие
  на сервере**, а не только светятся красным. Прежняя запись «тариф не поддерживает
  branch protection/rulesets» **была верна только для приватного репо** и снята 2026-08-22.
- ⚠️ `gh pr merge --auto` всё равно не использовать как ожидание гейта: он мержит до
  завершения checks. Порядок прежний — `gh pr checks <PR> --watch`, затем `gh pr merge --squash`.
  Серверная защита теперь страхует этот порядок, но не заменяет его.
