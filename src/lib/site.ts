...
/**
 * Яндекс.Метрика — решение владельца D-017 (мандат brain 2026-08-09):
 * счётчик на каждом публичном сайте + видимый информер в подвале.
 *
 * Счётчик заведён 2026-08-22 под аккаунтом владельца, домен внесён
 * ТОЛЬКО кириллицей (амендмент G136 от 10.08: Метрика отвергает punycode,
 * IDN она нормализует сама), галка «Принимать данные только с указанных
 * адресов» включена — без неё визиты режутся молча.
 *
 * Номер счётчика — не секрет, поэтому живёт в коде, а не в env: CI собирает
 * без прод-окружения, и NEXT_PUBLIC_-переменная запеклась бы пустой.
 */
export const ANALYTICS = {
  metrikaId: 111854549,
  /**
   * Разметка информера скопирована ИЗ КАБИНЕТА дословно (G237): самосборный
   * URL вида `<тип>_<стиль>_<фон>_<текст>_<стрелка>_<показатель>` не несёт
   * class="ym-advanced-informer" и data-cid, по которым тег Метрики находит
   * информер на странице, — и картинка навсегда остаётся нулевой.
   * Показатель `_0_uniques` — уникальные посетители (формулировка мандата).
   */
  informer: {
    src: "https://informer.yandex.ru/informer/111854549/3_1_FFFFFFFF_EFEFEFFF_0_uniques",
    href: "https://metrika.yandex.ru/stat/?id=111854549&from=informer",
