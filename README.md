...
- **Стек:** Next.js 15 (App Router) + TypeScript + Tailwind CSS 4, pnpm; стадия 1 — `output: 'export'` (статика, отдаётся nginx без Node-процесса)
...
- ✅ **LIVE** на общем боксе экосистемы (адрес и порт SSH — в secrets/vars репо, не в файлах: D-038): CI-деплой `deploy-prod.yml` — сборка в CI (G20), tar → releases/<sha> → symlink `current`, nginx-vhost `/etc/nginx/conf.d/rmz.conf` (раскладка бокса — conf.d, не sites-available), смоук #011 (200 + маркер + XFP-301).
...
- DNS: ✅ `рмз` + `*.рмз` → A-запись на бокс (панель хостера), консистентно на ns1-3 + 8.8.8.8.
- TLS: ⏳ выпуск LE из панели хостера пока падает — **петля прокси для свежего домена**, см. [docs/JINO-TLS-NOTES.md](docs/JINO-TLS-NOTES.md); повторять «Установить» в панели (SSL-вкладка домена).
