...
# синхронизацию с origin делает /start шаг 1 (#032), хук её не подменяет.
# Вызов: bash .claude/scripts/session_start.sh   (из .claude/settings.json → hooks.SessionStart)
cd "$(dirname "$0")/../.." || exit 0

echo "=== RmzMalmyzh · SessionStart · $(date +%F) ==="
...
echo
echo "--- Дальше: /start (синхра + почта brain + re-триаж PENDING). Канон: AGENTS.md ---"
exit 0
